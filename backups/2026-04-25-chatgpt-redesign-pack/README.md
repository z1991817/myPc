...
![v1 设计图](./assets/artimg-pro-ui-workbench-v1.png)

## 文件内容

- `assets/artimg-pro-ui-chatgpt-minimal-v2.png`
  当前推荐方向。白灰黑极简风，桌面端和移动端统一，适合小白直接上手。
- `assets/artimg-pro-ui-workbench-v1.png`
  第一版工作台方向图，保留作为对比参考，不作为主推方案。
...
1. 先把首页改成极简引导页，第一屏直接给输入框和示例提示词
2. 重做 `/create` 页面，改成对话流 + 图片结果流
3. 把比例、清晰度、风格、数量做成轻量参数胶囊
4. 把高级参数收进弹层
5. 接入任务状态：`提交中 / 生成中 / 成功 / 失败`
6. 加入结果图操作：下载、再次生成、继续编辑、复制提示词
7. 最后补移动端适配
